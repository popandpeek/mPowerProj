...
# CS 581 Autumn 2017
# 
# Rest feature extraction for training data: demographics:syn5511429 , table: syn5511449
...
actv_walking_syntable = syn.tableQuery(('SELECT "recordId", "healthCode", "deviceMotion_walking_rest.json.items" FROM {0} WHERE healthCode IN ({1}) AND "deviceMotion_walking_rest.json.items" is not null').format(INPUT_WALKING_ACTIVITY_TABLE_SYNID, healthCodeList))
...
walk_json_files = syn.downloadTableColumns(actv_walking_syntable, "deviceMotion_walking_rest.json.items")
items = walk_json_files.items()

# create pandas dataframe of JSON filepaths and filehandleIDs
walk_json_files_temp = pd.DataFrame({"deviceMotion_walking_rest.json.items": [i[0] for i in items], "rest_walk_json_file": [i[1] for i in items]})
...
reduced_data_set.to_csv('mPowerFeatrues_rest_1205_reduced.csv')